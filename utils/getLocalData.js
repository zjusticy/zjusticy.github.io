...
export function getPostSlugs(mode) {
  return mode === 'posts' ? fs.readdirSync(postsDirectory) : fs.readdirSync(resumeDirectory)
}

export function getPostBySlug(slug, fields = [], mode) {
  const realSlug = slug.replace(/\.md$/, '');
  const fullPath = join(mode === 'posts' ? postsDirectory : resumeDirectory, `${realSlug}.md`);
...
export function getAllPosts(fields = [], mode) {
  ...
  const posts = slugs
    .map(slug => getPostBySlug(slug, fields, mode));
    // sort posts by date in descending order
    
  const order_posts = (mode === "posts") ? posts.sort((post1, post2) => (post1.date > post2.date ? '-1' : '1')) :
